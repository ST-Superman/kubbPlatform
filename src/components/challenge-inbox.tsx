...
/** Full challenge center for /challenges: incoming (accept/decline) + outgoing (cancel). */
export function ChallengeInbox({ initial }: { initial: Challenge[] }) {
  ...
  const incoming = useMemo(() => items.filter((c) => c.direction === "incoming"), [items]);
  const outgoing = useMemo(() => items.filter((c) => c.direction === "outgoing"), [items]);
      ...
      <div className="flex items-center gap-3 rounded-2xl border border-border bg-card px-4 py-3 shadow-sm">
        <div className="min-w-0 flex-1">
          <div className="truncate text-sm font-semibold">
            {c.direction === "incoming" ? `${name} challenged you` : `Waiting on ${firstName(c.other_display_name)}`}
          </div>
    ...
    <div className="flex flex-col gap-5">
      {incoming.length > 0 ? (
        <div className="flex flex-col gap-2">
          <span className="eyebrow text-[10px] tracking-[1.5px] text-muted-foreground">INCOMING</span>
          {incoming.map((c) => (
            <Row key={c.id} c={c} />
          ))}
        </div>
      ...
      {outgoing.length > 0 ? (
        <div className="flex flex-col gap-2">
          <span className="eyebrow text-[10px] tracking-[1.5px] text-muted-foreground">SENT</span>
          {outgoing.map((c) => (
            <Row key={c.id} c={c} />
          ))}
        </div>
